...
- **Atoms are canvas siblings of bubbles**, never children. This lets the user drag an atom out of its subtopic into another or into floater state. See [`WorkshopCanvas.tsx`](src/components/canvas/WorkshopCanvas.tsx) `DraggableAtom`.
...
- **`instant=true` propagation during group drags**. When a subtopic, topic, or cluster is being dragged, all member atoms get `instant=true` so their useEffect calls `x.set()` instead of `animate(...)`. Without this, hundreds of 0.45s tweens spawn per second and the canvas freezes on release. See `draggingSubtopicId`, `draggingTopicId`, `draggingClusterMemberIds` state.
...
- **Proximity clusters are computed every render**. `detectProximityClusters(atomRecords, 240)` BFS-groups floaters within 240 px sharing the same `topic_id`. Cluster id is `cluster-${[...ids].sort().join("-")}` so React reconciles stably as long as members don't change.
...
| **Reaction edges only between two atoms in the same non-null subtopic.** Floaters / cross-subtopic atoms have NO edges. | "拖出 subtopic 后连线立即断开" requirement |
...
| 4 | `SubtopicBubble` (squircle, in-place morph) | [src/components/canvas/SubtopicBubble.tsx](src/components/canvas/SubtopicBubble.tsx) | Same DOM element morphs collapsed↔expanded via framer animate(width/height). Topology preview is rendered by canvas-sibling atoms (preview dots) — bubble itself is just the backdrop. |
...
- **Don't render atoms inside the subtopic's DOM tree.** Atoms are canvas-level siblings; this is what enables drag-out.
