...
- **Proximity clusters are computed every render**. `detectProximityClusters(atomRecords, 240)` BFS-groups floaters within 240 px sharing the same `topic_id`. Cluster mechanism is uniform across fixture and custom (crystallized) topics — to prevent crystallized topics from auto-wrapping their members in a subtopic candidate on placement, `onClusterCrystallize` spreads members past the proximity threshold (see §4 row "Cluster mechanism is uniform"). Cluster id is `cluster-${[...ids].sort().join("-")}` so React reconciles stably as long as members don't change.
- **Drag-overlap halo (subtopic-preview gesture)**. While the user is dragging a floater atom, `dragHaloTargetId` finds another floater in the SAME topic whose AABB overlap area exceeds 30 % of the dragged atom's area. When set, **a SINGLE dashed amber squircle (28 px outer padding to match `ClusterBubble`) is rendered as a sibling of the atom layer at the bbox of both atoms** — preview of the subtopic they would form. On release: the dropped atom is laid SIDE-BY-SIDE with the target (32 px gap) and INHERITS the target's `topic_id`; both atoms are marked `fixed` in the collision resolver so they stay where placed; proximity catches them; `ClusterBubble` appears at the same place the halo just disappeared from. See [`WorkshopCanvas.tsx`](src/components/canvas/WorkshopCanvas.tsx) `dragHaloTargetId` memo + the halo render block above the atom map + the atom drag-end handler.
- **Reaction edges use `rectExit(...)` to anchor at each card's RECT EDGE**, not center. Geometry helper in [`ReactionEdge.tsx`](src/components/canvas/ReactionEdge.tsx) — used by both `ReactionLayer` (compact cards) and `SubtopicBubble`'s preview band (preview dots). Outward `gap` (3 / 1 px) keeps the line off the card border for visual breathing room.
- **Reaction-edge tooltip is custom (not native `<title>`)**. `ReactionEdge.onHover` propagates `clientX/Y`; `WorkshopCanvas` owns `edgeHover` state and renders a `position: fixed` tooltip card outside the camera transform. No OS hover delay, follows the cursor, clamped to viewport.
...
| **Reaction edges anchor to each atom's RECT EDGE, not its center.** `rectExit(cx, cy, halfW, halfH, dx, dy, gap)` in [`ReactionEdge.tsx`](src/components/canvas/ReactionEdge.tsx) computes where the line from one atom's center to the other exits the source rectangle, plus a small outward `gap` (3 px for compact cards, 1 px for preview dots) so the line breathes off the card border. Reads as a connection between cards, not a line piercing through them. | User direction (2026-05-09) |
| **Reaction edges show a custom hover tooltip** (label + description + AI-suggested marker for ghost keys). NOT native SVG `<title>` — that waits ~500 ms for the OS hover delay and felt sluggish. Implementation: `ReactionEdge.onHover` propagates `clientX/Y` up via `ReactionLayer.onEdgeHover`; `WorkshopCanvas` renders a `position: fixed` tooltip card outside the camera transform that follows the cursor with no delay (clamped to viewport). | User direction (2026-05-09) |
...
| **Heavy floater-on-floater overlap during drag shows a single dashed amber squircle that frames BOTH atoms** (the preview of the subtopic they would form). Halo only shows for two floaters in the same topic. On release, the dropped atom is laid SIDE-BY-SIDE with the halo target (32 px gap) and INHERITS the target's `topic_id` — guarantees the cluster bbox is visibly ~2 cards wide AND that same-topic membership is locked in even if the target's visual position drifted past the topic vessel cap. The halo's footprint exactly matches `ClusterBubble`'s outer padding (28 px) so the preview "settles" into the committed cluster, not jumps. | User direction (2026-05-08, 2026-05-09) |
| **Cluster mechanism is uniform across fixture and custom topics.** `detectProximityClusters` no longer skips custom-topic atoms — the user expects "drag two atoms together → subtopic cluster" to work everywhere. To prevent freshly-crystallized topics from auto-clustering on placement, `onClusterCrystallize` (topic case) spreads members with HGAP=110 / VGAP=160 so center-to-center > 240 px (the BFS threshold). The user can still drag any two atoms inside the new Topic closer to trigger a subtopic cluster. | User direction (2026-05-09) |
...
| **Preview dots show author initials.** Human-atom preview dots (18×18 inside the collapsed bubble) overlay 1-2 letters from the author's name (8.5 px JetBrains Mono, white with subtle text-shadow). The colored swatch identifies voice across the workshop; the initials disambiguate similar-coloured contributors at a glance. | User direction (2026-05-09) |
